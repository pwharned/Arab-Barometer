...
List2 = split_listA_is_TWO%>%
  map(nice_function_2)%>%
  set_names(map_chr(split_listA_is_TWO, paste))
...
List1= split_listA_is_on1%>%
  map(nice_function_1)%>%
  set_names(map_chr(split_listA_is_on1, paste))

###
View(List1[["Q604B_3"]])
### Large amounts of observations for both countries where splita ==2 and Q604B_3!=NA
###Q204C_15 and Q204C_131§   observations in all countries for splita ==2