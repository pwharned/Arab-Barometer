...
abv%>%
  ...
  filter_at(vars(starts_with("Q108")), any_vars(.%in%c(NA)))%>%
  filter(splita==1)%>%
  mutate_at(vars(starts_with("Q108")), function(x)x=ifelse(x%in%c(NA), 99, x))%>%
  select(starts_with("Q108"))
...
abv%>%
  filter(country%in%c( 22, 15, 7))%>%
  filter_at(vars(starts_with("Q201B")), any_vars(.%in%c(NA)))%>%
  filter(splita==1)%>%
  mutate_at(vars(starts_with("Q201B")), function(x)x=ifelse(x%in%c(NA), 99, x))%>%
  select(starts_with("Q201B"))



abv%>%
  filter(splita==2&country!=9)%>%
  filter_at(vars(starts_with("Q201C")), any_vars(.%in%c(NA)))%>%
  mutate_at(vars(starts_with("Q201C")), function(x)x=ifelse(x%in%c(NA), 99, x))%>%
  select(country, id, starts_with("Q201C"))

#Q211 Lebanon,Palestine if Q211=SYSMIS and Q210 in (1,2,3) -> 99. Libya dirty code 0->sysmis (filter). 
abv%>%
  filter(Q211%in%c(NA)&Q210%in%c(1,2,3))%>%
  mutate(Q211 = ifelse(Q211%in%c(NA), 99, Q211))

###Palestine,Yemen: incorrect filter, some answers for q210=not at all, dk,refused


abv%>%
  filter(Q211%nin%c(NA)&Q210%nin%c(1,2,3))%>%
...
abv%>%
  filter(Q211A%in%c(NA)&Q210%in%c(1,2,3))%>%
...
abv%>%
  filter(country%in%c(22))%>%
  filter(Q301A%nin%c(NA)&splita==2)%>%
  mutate(Q301A = ifelse(Q301A%nin%c(NA)&splita==2, NA,Q301A ))%>%
  select(Q301A)
...
abv%>%
  filter(country%in%c(22))%>%
  filter(Q301B%nin%c(NA)&splita==1)%>%
  mutate(Q301B = ifelse(Q301B%nin%c(NA)&splita==1, NA,Q301B ))%>%
  select(Q301B)
...
abv%>%
  filter(country ==22)%>%
  filter(Q514%in%c(NA)&splita==1)%>%
  mutate(Q514 = ifelse(Q514%in%c(NA)&splita==1, 99,Q514 ))%>%
  select(Q514)
  
#Q514A, Iraq, Palestine, Yemen if Q514A=sysmis and splita=2 ->99. 
abv%>%
  filter(country %in%c(7,22,15))%>%
  filter(Q514A%in%c(NA)&splita==2)%>%
  mutate(Q514A = ifelse(Q514A%in%c(NA)&splita==2, 99,Q514A ))%>%
  select(Q514A)
...
abv%>%
  filter(country %in%c(7,22,15))%>%
  filter(Q604A_1%in%c(NA)&splita==1)%>%
  mutate(Q604A_1 = ifelse(Q604A_1%in%c(NA)&splita==1, 99,Q604A_1 ))%>%
  select(Q604A_1)
...
abv%>%
  filter(country %in%c(7,22,15))%>%
  filter(Q604A_3%in%c(NA)&splita==1)%>%
  mutate(Q604A_3 = ifelse(Q604A_3%in%c(NA)&splita==1, 99,Q604A_3 ))%>%
  select(Q604A_3)

#Q701C_2 Q701C_4 Iraq if Q701C_xxx=sysmis and splita=1-> 99 refused

abv%>%
  filter(country %in%c(7)&splita==1)%>%
  filter_at(c("Q701C_2","Q701C_4"), any_vars(.%in%c(NA)))%>%
  mutate(Q701C_2 = ifelse(Q701C_2%in%c(NA)&splita==1, 99,Q701C_2 ))%>%
  mutate(Q701C_4 = ifelse(Q701C_4%in%c(NA)&splita==1, 99,Q701C_4 ))%>%
  select(Q701C_2,Q701C_4)
...
abv%>%
  filter(country %in%c(22)&splita==2)%>%
  filter_at(c("Q514"), any_vars(.%nin%c(NA)))%>%
  mutate(Q514 = ifelse(Q514%nin%c(NA)&splita==2, NA,Q514 ))%>%
  select(Q514)

#Q514A : Yemen incorrect filter. some answers for splita=1
abv%>%
  filter(country %in%c(22)&splita==1)%>%
  filter_at(c("Q514A"), any_vars(.%nin%c(NA)))%>%
  mutate(Q514A = ifelse(Q514A%nin%c(NA)&splita==1, NA,Q514A ))%>%
  select(Q514)

#Q602_4A Lebanon asked only to Muslim! No answers for 1 Strongly dislike!
  
abv%>%
  filter(Q602_4A%nin%(NA)&Q1012!=1)%>%
  mutate(Q602_4A=ifelse(Q602_4A%nin%(NA)&Q1012!=1, NA,Q602_4A))%>%
  select(Q602_4A,Q1012,Q1012A)


#Q602_4B Filter?? SYSMIS=99? suposed to be filtered by religious sect
  
abv%>%
  filter(Q602_4B%nin%(NA)&Q1012!=2)%>%
...
abv%>%
  filter(country%in%c(15,22))%>%
  filter(Q604B_1%in%(NA)&splita==2)%>%
  mutate(Q604B_1=ifelse(Q604B_1%in%(NA)&splita==2, 99,Q604B_1))%>%
  select(Q604B_1)
  
#Yemen incorrect filter.Q604B_1 some answers for splita=1 
abv%>%
  filter(country%in%c(15,22))%>%
  filter(Q604B_1%nin%(NA)&splita==1)%>%
  mutate(Q604B_1=ifelse(Q604B_1%nin%(NA)&splita==1, NA,Q604B_1))%>%
  select(Q604B_1)
...
abv%>%
  filter(country%in%c(15,22,7))%>%
  filter(Q604B_3%nin%(NA)&splita==1)%>%
  mutate(Q604B_3=ifelse(Q604B_3%nin%(NA)&splita==1, NA,Q604B_3))%>%
  select(Q604B_3)


#Q605 Lebanon, Asked only to Muslim - affects one observation
abv%>%
  filter(Q605%nin%c(NA)&Q1012!=1)%>%
...
abv%>%
  filter_at(vars(starts_with("Q605")), any_vars(.%in%c(NA)))%>%
  filter(splita==2&Q1012==1&country!=9)%>%
  mutate(Q605A_1=ifelse(Q605A_1%in%c(NA)&splita==2&Q1012==1, 99,Q605A_1))
  mutate(Q605A_2=ifelse(Q605A_2%in%c(NA)&splita==2&Q1012==1, 99,Q605A_2))
  mutate(Q605A_3=ifelse(Q605A_3%in%c(NA)&splita==2&Q1012==1, 99,Q605A_3))
  mutate(Q605A_4=ifelse(Q605A_4%in%c(NA)&splita==2&Q1012==1, 99,Q605A_4))
  mutate(Q605B=ifelse(Q605B%in%c(NA)&splita==2&Q1012==1, 99,Q605B))
  mutate(Q605=ifelse(Q605%in%c(NA)&splita==2&Q1012==1, 99,Q605))

#Yemen incorrect filter. some answers for splita=2 - No need to recode as the whole sample received the quesiton
#  Q607_7 Lebanon, 
  
  abv%>%
    filter(Q607_7%nin%c(NA)&Q1012!=1&Q1012A!=10)%>%
...
#Yemen incorrect filter, some answers for splita=1
  ...
  abv%>%
    filter(Q703%nin%c(NA)&splita==2&country!=9)%>%
...
# Yemen incorrect filter, some answers for splita=1
  ...
  abv%>%
    filter(Q705%nin%c(NA)&splita==1&country!=9)%>%
...
#Q7141A Iraq,Yemen if if Q7141A =sysmis and splita=1 ->99 refused. Kuwait no filter. 
  ...
  abv%>%
    filter(Q7141A%in%c(NA)&splita==1)%>%
...
abv%>%
  filter(Q7141A%nin%c(NA)&splita==2&country!=9)%>%
...
abv%>%
  filter(Q7141B%in%c(NA)&splita==2&country!=9)%>%
...
abv%>%
  filter(Q7141B%nin%c(NA)&splita==1&country!=9)%>%
...
abv%>%
  filter(Q851B%in%c(NA)&Q851A==1&country!=9)%>%
...
abv%>%
  filter(Q851B%nin%c(NA)&Q851A!=1&country!=9)%>%
...
abv%>%
  filter(Q851C%in%c(NA)&Q851B==1&country!=9)%>%
...
abv%>%
  filter(Q852%nin%c(NA)&splita==1&country!=9)%>%
...
abv%>%
  filter(Q853A%in%c(NA)&splita==2&Q1002==1&country==13)%>%
...
abv%>%
  filter(Q853A%nin%c(NA)&splita==2&Q1002==2&country==7)%>%
...
abv%>%
  filter(Q854%in%c(NA)&splita==2&country%in%c(7,13))%>%
  mutate(Q854= ifelse(Q854%in%c(NA)&splita==2&country%in%c(7,13), 99,Q854))%>%
  select(Q854)
  
abv%>%
  filter(country==15, Q701C_2%in%c(NA), splita==1&mode==2)%>%
  select(Q701C_2)

#Kuwait no filter. Yemen, few incorrect answers for splita=1
abv%>%
  filter(Q854%nin%c(NA)&splita==1&country!=9)%>%
